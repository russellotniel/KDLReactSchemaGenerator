...
interface Props {
	features: string;
	handleStep4: (tempDDL: string) => void;
}

const SchemaEditor: React.FC<Props> = ({ features, handleStep4 }) => {
	...
	return (
		<div className='d-flex vh-100'>
			...
			<Modal show={open} onHide={handleClose} backdrop='static' keyboard={false}>
				...
				<Modal.Body>
					<textarea value={ddl} disabled style={{ height: '30vh', width: '100%' }} />
				</Modal.Body>
				<Modal.Footer>
					...
					<Button variant='success' onClick={() => handleStep4(ddl)}>
						Next Page
					</Button>
					{/* <Button variant='success' onClick={handleCopy}>Copy to Clipboard</Button> */}
				</Modal.Footer>
			</Modal>
		</div>
	);
