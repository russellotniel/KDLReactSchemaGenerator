export const UserStoriesGenerator = async (chatMessages: string) => {
  ...
  const systemMessage = {
    role: "system",
    content:
      'Users want to make an App. The response must be in bullet points without preamble. The bullet points change with *. Not Accepting "-" bullet points. The points no less than five',
  };
  ...
  for (const line of lines) {
    ...
    if (trimmedLine.startsWith("CREATE TABLE")) {
      ...
      currentTable.foreignKeys.push(
        `${currentTable.tableName} ||--o{ ${referencedTable} : ""`
      );
